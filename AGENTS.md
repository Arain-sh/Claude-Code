...
This file provides guidance to contributors and coding agents working in Arain's fork of this repository.

## Project Overview

This is Arain's **reverse-engineered / decompiled** CLI project based on Anthropic's official coding assistant tooling. The goal is to restore core functionality while trimming secondary capabilities. Many modules are stubbed or feature-flagged off. The codebase has ~1341 tsc errors from decompilation (mostly `unknown`/`never`/`{}` types) — these do **not** block Bun runtime execution.
